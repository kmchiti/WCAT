...
def _resnet(
    ...
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls[arch], save_path,
...
@Base_Model.register('resnet18')
class Resnet18(Base_Model, ResNet):
    def __init__(self, num_classes=1000, pretrained=False, progress=True,
                 **kwargs):
        ResNet.__init__(self, block=BasicBlock, layers=[2, 2, 2, 2], num_classes=num_classes)
        Base_Model.__init__(self, **kwargs)
        if pretrained:
            state_dict = load_state_dict_from_url(pretrained_weight_urls['resnet18'], self.save_path,
...
@Base_Model.register('resnet34')
class Resnet34(Base_Model, ResNet):
    def __init__(self, num_classes=1000, pretrained=False, progress=True,
                 **kwargs):
        ResNet.__init__(self, block=BasicBlock, layers=[3, 4, 6, 3], num_classes=num_classes)
        Base_Model.__init__(self, **kwargs)
        if pretrained:
            state_dict = load_state_dict_from_url(pretrained_weight_urls['resnet34'], self.save_path,
                                                  map_location=self.device, progress=progress)
            network_kvpair = self.state_dict()
            for key in state_dict.keys():
                network_kvpair[key] = state_dict[key]
...
@Base_Model.register('resnet50')
class Resnet50(Base_Model, ResNet):
    def __init__(self, num_classes=1000, pretrained=False, progress=True,
                 **kwargs):
        ResNet.__init__(self, block=Bottleneck, layers=[3, 4, 6, 3], num_classes=num_classes)
        Base_Model.__init__(self, **kwargs)
        if pretrained:
            state_dict = load_state_dict_from_url(pretrained_weight_urls['resnet34'], self.save_path,
                                                  map_location=self.device, progress=progress)
